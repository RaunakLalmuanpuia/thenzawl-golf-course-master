            ...
            <v-row>
                ...
                <v-col style="margin-top: 1rem">
                    <v-btn outlined elevation="0" color="blue" icon
                        @click="getHistory()"><v-icon>mdi-magnify</v-icon></v-btn>
                </v-col>
            </v-row>
        </v-card-title>
        <v-data-table :headers="headers" :items="history" :options.sync="options"
        ...
        getHistory() {
            ...
            if (self.options.searchBy == "period") {
                self.$axios
                    ...
                    .then(function (res) {
                        self.history = res.data.history.data;
                        self.options.totalBooked = res.data.history.total;
                        self.options.page = res.data.history.current_page;
                        self.loading = false;
                    });
            } else {
                self.$axios
