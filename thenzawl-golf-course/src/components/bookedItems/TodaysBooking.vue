            ...
            <v-row>
                ...
                <v-col style="margin-top: 1rem">
                    <v-btn outlined elevation="0" color="blue" icon
                        @click="getBooked()"><v-icon>mdi-magnify</v-icon></v-btn>
                </v-col>
            </v-row>
        </v-card-title>
        <v-data-table :headers="headers" :items="orders" :options.sync="options"
        ...
        getBooked() {
            ...
            if (self.options.searchBy == "period") {
                self.$axios
                    ...
                    .then(function (res) {
                        self.orders = res.data.orders.data;
                        self.options.totalBooked = res.data.orders.total;
                        self.options.page = res.data.orders.current_page;
                        self.loading = false;
                    });
            } else {
                self.$axios
