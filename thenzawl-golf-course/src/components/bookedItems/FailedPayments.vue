            ...
            <v-row>
                ...
                <v-col style="margin-top: 1rem">
                    <v-btn outlined elevation="0" color="blue" icon
                        @click="getFailed()"><v-icon>mdi-magnify</v-icon></v-btn>
                </v-col>
            </v-row>
        </v-card-title>
        <v-data-table :headers="headers" :items="failed" :options.sync="options"
        ...
        getFailed() {
            ...
            if (self.options.searchBy == "period") {
                self.$axios
                    ...
                    .then(function (res) {
                        self.failed = res.data.failedPayments.data;
                        self.options.totalBooked = res.data.failedPayments.total;
                        self.options.page = res.data.failedPayments.current_page;
                        self.loading = false;
                    });
            } else {
                self.$axios
