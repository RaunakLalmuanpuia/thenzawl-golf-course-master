<template>
  <v-card flat style="background-color: #f7f9f7">
    <v-card-title>Videography</v-card-title>
    <v-card-text>
      <v-row>
        <v-col md="6">
          <v-menu
      ...
      <v-row v-if="date_from != ''">
        <v-col cols="12">
          <v-label>No. Of Person (Excluding first five PAX)</v-label>
        </v-col>
        <v-col cols="12" md="4">
...
export default {
  ...
  methods: {
    ...
    proceedBooking() {
      this.totalPricePerson = this.numberOfPerson * 200;
