<template>
  <v-container fluid ml-6 mt-6>
    ...
    >
      <template v-slot:top>
        <v-toolbar flat>
          <v-toolbar-title>Places Of Interest</v-toolbar-title>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Description", value: "address", sortable: false },
  ...
  methods: {
    edit(id) {
      let route = "list-place-of-interest/" + id + "/edit";
    ...
    getPlaceOfInterest() {
      ...
      self.show = true;
      self.$axios
        .get("get-place-of-interest/")
        .then((res) => {
          self.items = res.data.placeOfInterests;
          self.show = false;
        })
        .catch((err) => {
          self.show = false;
    ...
    confirmDelete() {
      ...
      self.show = true;
      this.$axios
        .post("delete-place-of-interest/" + self.deleteId)
        .then(function (res) {
          self.show = false;
